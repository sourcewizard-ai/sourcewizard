...
import { renderInstall, renderMCPStatus } from "./install.js";
...
export class MCPPackageCLI {
  ...
  private setupCommands(): void {
    ...
    this.program
      ...
      .option("--list-packages", "List all available packages")
  ...
  async handleListPackages(): Promise<void> {
    try {
      const serverUrl = process.env.SOURCEWIZARD_SERVER_URL ||
        (process.env.NODE_ENV === "development" ? "http://localhost:3000" : "https://sourcewizard.ai");

      const response = await fetch(`${serverUrl}/api/packages/list`);
      const result = await response.json();

      if (!result.success) {
        console.error(chalk.red("Failed to fetch packages:"), result.error);
        process.exit(1);
      }
      ...
      for (const pkg of packages) {
        console.log(chalk.cyan(`• ${pkg.name}`) + chalk.gray(` [${pkg.language}]`));
      ...
      console.log(chalk.gray(`Total: ${packages.length} package${packages.length !== 1 ? 's' : ''}`));
    } catch (error) {
      console.error(
        chalk.red("Failed to list packages:"),
        error instanceof Error ? error.message : String(error)
      );
      process.exit(1);
    }
