...
5. Use tree-shaking compatible imports
  ...
  async installPackage(options: InstallationOptions): Promise<InstallationResult> {
    try {
      ...
      if (!pkg && !snippet) {
        return {
          ...
          errors: [`No package or snippet found with name: ${options.packageName}`]
        };
      }
  ...
  private async installNpmPackage(
    ...
    try {
      ...
      const aiInstructions = this.getAIInstructions(pkg, projectContext, options);
      ...
      if (aiInstructions) {
        const setupResult = await this.applyAISetup(pkg, aiInstructions, projectContext);
      ...
      result.message = `Installation failed: ${error instanceof Error ? error.message : String(error)}`;
      return result;
    }
  }
  ...
  private async installCodeSnippet(
    ...
    try {
      ...
      if (snippet.usageExample) {
        const usageFile = filePath.replace(path.extname(filePath), '.example' + path.extname(filePath));
      ...
      result.errors?.push(error instanceof Error ? error.message : String(error));
      result.message = `Code snippet installation failed: ${error instanceof Error ? error.message : String(error)}`;
      return result;
    }
  }
  ...
  private detectPackageManager(context: ProjectContext): 'npm' | 'yarn' | 'pnpm' | 'bun' {
  ...
  private async applyAISetup(
    ...
    result.warnings.push(`AI instructions available for ${pkg.name}. Check console for details.`);
    
    return result;
  }

  private generateSnippetFileName(snippet: CodeSnippet, context: ProjectContext): string {
