...
const MainMenuScreen: React.FC<{
...
}> = ({ onSelect, selectedIndex, onNavigate }) => {
  ...
  useInput((input, key) => {
    if (key.upArrow) {
      ...
      onSelect(mainMenuOptions[selectedIndex].action);
    }
  });
      ...
      <Box flex={1} justifyContent="center" alignItems="center">
        ...
        >
          ...
          <SimpleSelect
            ...
            onSelect={(option) => onSelect(option.action)}
          />
          <Text> </Text>
          <Text color="gray">
...
const PackageSelectionScreen: React.FC<{
  ...
  onNavigate: (direction: number) => void;
}> = ({ packages, selectedIndex, onSelect, onNavigate }) => {
  ...
  const packageOptions: MenuOption[] = packages.map((pkg) => ({
    key: pkg,
    label: pkg,
  ...
  useInput((input, key) => {
    if (key.upArrow) {
      ...
      onSelect(packages[selectedIndex]);
    }
  });
...
const App: React.FC = () => {
  ...
  const handleNavigate = (direction: number) => {
    setState((prev) => {
      const maxIndex =
        prev.screen === "main-menu" ? 6 : prev.packages.length - 1;
      const newIndex = Math.max(
        0,
        Math.min(maxIndex, prev.selectedIndex + direction)
      );
  ...
  const handleWelcomeContinue = () => {
    setState((prev) => ({ ...prev, screen: "main-menu", selectedIndex: 0 }));
  };
...
export const DOSSetupWizardInkSimple = {
  start: () => {
    render(<App />);
  },
};
